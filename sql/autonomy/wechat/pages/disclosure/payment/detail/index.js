...
function mapBuildingView(building) {
...
function buildSummary(buildings) {
...
function buildBuildingTabs(buildings) {
...
function filterBuildingHouses(building, keyword) {
    if (!building) {
        return null;
    }
    ...
    const houses = building.houses.filter((house) => [house.houseLabel, house.displayName, house.roomNo, house.unitNo]
        .filter(Boolean)
...
function getSidebarResult(event) {
    return {
...
Page({
    ...
    async loadDetail(periodKey) {
        this.setData({
            loading: true,
            errorMessage: '',
        });
        try {
            const result = await (0, management_fee_1.fetchManagementFeeDisclosureTree)({ periodKey });
            ...
            this.setData({
                ...
                periodMeta: period.dueDate ? `截止日期 ${(0, management_fee_1.formatManagementFeeDate)(period.dueDate)}` : '未设置截止日期',
    ...
    handleKeywordInput(event) {
        ...
        const activeBuilding = this.data.rawBuildings.find((item) => item.buildingId === this.data.activeBuildingId) || null;
        this.setData({
            keyword,
    ...
    handleSideBarChange(event) {
        ...
        const activeBuilding = this.data.rawBuildings.find((item) => item.buildingId === selected.value) || this.data.rawBuildings[0] || null;
        this.setData({
            activeBuildingId: activeBuilding?.buildingId || '',
