...
function useList(options) {
    ...
    const { query, transform, immediate = true, selectId = (item) => item.id } = options;
    let payload;
    let unsubscribe;
    let requestToken = 0;
    const emitter = new EventEmitter_1.default();
    async function fetchPage(type, offset = 0) {
    ...
    return Behavior({
        ...
        methods: {
            ...
            async onListInit(queryPayload, options) {
