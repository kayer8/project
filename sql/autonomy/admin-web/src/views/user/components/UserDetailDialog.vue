  ...
  >
    <div class="dialog-body">
      <div class="dialog-toolbar">
        <div class="dialog-desc">查看账号基础信息、社区角色、房屋关系和认证申请记录。</div>
      ...
      <t-card v-if="loading" title="加载中">
        <div class="empty-text">正在加载用户详情...</div>
      </t-card>
      ...
      <t-card v-else-if="!user" title="未找到用户">
...
async function loadDetail() {
  ...
  loadError.value = '';
  try {
    user.value = await fetchUserDetail(props.detailId);
  } catch (error) {
    user.value = null;
